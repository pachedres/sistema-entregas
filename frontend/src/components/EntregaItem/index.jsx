...
const API_RASTREAMENTO_BASE =
  import.meta.env.VITE_RASTREAMENTO_API_BASE || "http://localhost:8003";
const API_NOTIFICACOES_BASE =
  import.meta.env.VITE_NOTIFICACOES_API_BASE || "http://localhost:8004";
...
export default function EntregaItem({ entrega, onIniciar, onFinalizar }) {
    ...
    const carregarResumo = async () => {
      try {
        const [rastreamentoRes, notificacoesRes] = await Promise.all([
          ...
          fetch(
            `${API_NOTIFICACOES_BASE}/notificacoes/?pedido_id=${entrega.pedido_id}`,
          ),
        ]);
        ...
        if (rastreamentoRes.ok) {
          ...
          if (Array.isArray(rastreamento) && rastreamento.length > 0) {
            ...
            setUltimoEvento(
              eventoMaisRecente?.evento || "Nenhum evento registrado",
            );
          } else {
        ...
        if (notificacoesRes.ok) {
          ...
          if (Array.isArray(notificacoes) && notificacoes.length > 0) {
            ...
            setUltimaNotificacao(ultima || null);
          }
        }
      } catch (err) {
        console.error(err);
  ...
  return (
    <div
      className={`entrega-item ${entrega.status === "ENTREGUE" ? "entrega-finalizada" : entrega.status === "EM_ENTREGA" ? "entrega-em-entrega" : ""}`}
    >
      <div className="info">
        <div>
        ...
        <div>
          ...
          <span className={`status ${entrega.status}`}>{entrega.status}</span>
        </div>
        <div>
          <strong>Entregador:</strong>{" "}
          {entrega.entregador_nome || "Não atribuído"}
        </div>
        <div>
          <strong>Último evento:</strong> {ultimaNotificacao?.evento_origem}
        </div>
        {ultimaNotificacao && (
          <div>
            <strong>Última notificação:</strong> {ultimaNotificacao.mensagem}
          </div>
        )}
      </div>
